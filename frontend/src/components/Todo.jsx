...
function Todo({ index, item }) {
  ...
  const deleteTodoList = async (item) => {
    try {
      const {
        ...
        `http://localhost:8080/api/${id}/delete?data=${item}`
      );
      UserDispatch({
        type: "ADD_TODO",
        payload: data,
  ...
  return (
    <div
      className={`flex justify-between m-8 p-2 rounded-xl ${
        !checked ? "bg-slate-600/20" : "bg-blue-300"
      }`}
    >
      <input type="checkbox" onChange={handleCheckInput} checked={checked} />
      <p
        className={` font-semibold ${
          !checked ? "text-white" : "line-through text-black"
        }`}
        key={index}
      >
        {item}
